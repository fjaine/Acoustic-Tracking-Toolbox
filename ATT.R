### Acoustic Tracking Toolbox
###
### IMOS ATF species comparison analysis
### script written by Vinay Udyawer and the IMOS Task Team 
### Function uses QC IMOS tag data or VUE exports to summarise movement and acitivity space metrics
...
ATT<-function(tagdata, taginfo, IMOSdata=FALSE, sig2=200, timestep=60, extent=2, grid=200, sub="%Y-%m", cumulative=FALSE, plotfull=FALSE, plotsub=FALSE, storepoly=FALSE, QCval=2){
  ...
  ### extent:     Extent metric used to define extent of grid for brownian bridge kud estimation [default= 2]
  ...
  ####################################################################################################################################################
  ##### Setup libraries
  sapply(c("maptools", "maps", "adehabitatHR", "sp", "raster", "rasterVis", "plyr", "lubridate"), require, character.only=TRUE)
  ...
  if(nrow(data)>1){
    message('Acoustic Tracking Toolbox: Calculating standardised metrics for tag ', as.character(data$transmitter_id[1]) )
    ##### Setup data
    ll<-CRS("+proj=longlat +datum=WGS84"); utm<-CRS("+init=epsg:3577")
    data$dt<-as.POSIXct(strptime(data$detection_timestamp, format="%Y-%m-%d %H:%M:%S"), tz="GMT")
    ...
    if(nrow(unique(COA[,c("meanlat","meanlon")]))>5){
      ...
      gr<-expand.grid(x=seq(xcen-(width*extent), xcen+(width*extent),len=grid),y=seq(ycen-(width*extent), ycen+(width*extent),len=grid)); coordinates(gr) <- ~x+y; gridded(gr)<-TRUE 
      
      ### Full data
      tf<-as.ltraj(xy=coordinates(dat), date=ymd_hms(dat$DateTime, tz="GMT"), id=dat$transmitter_id, typeII=TRUE, proj4string=utm)
      s1f<-(liker(tf, rangesig1=c(0,500), sig2=sig2, byburst=FALSE, plotit=FALSE)[[1]]$sig1)/4
      tryCatch({
      ...
      if(cumulative){
        ...
        for(c in 1:length(levels(dat$yearmon))){
          ...
          csig1<-(liker(ct, rangesig1=c(0,500), sig2=sig2, byburst=FALSE, plotit=FALSE)[[1]]$sig1)/4
          tryCatch({
